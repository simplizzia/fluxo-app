...
import { actionContinuarBriefingMarca } from '../../onboarding-actions'
...
const CATEGORIAS_CONTEUDO = [
  ...
  { value: 'campanhas',   label: 'Campanhas',              placeholder: 'Plataforma de marca, campanha mestre, estrutura Hero/Hub/Help...' },
  { value: 'calendario',  label: 'Calendário Editorial',   placeholder: 'Calendário mensal aprovado, momentos-chave, ativações...' },
]
...
export default function MarcaTabSingle({ clienteId, marcaId, secoes, ativos, moodboard, podeEditar, insights }: Props) {
  ...
  // Briefing de marca gerado pela Izzi para esta marca
  const briefingsMarca = secoes.filter((s) => s.subcategoria === 'briefing_marca')

  // Seções editáveis (exclui o briefing gerado pela Izzi)
  const secoesEditaveis = secoes.filter((s) => s.subcategoria !== 'briefing_marca')
    ...
    <div className="space-y-5">
      ...
      {subTab === 'estrategia' && (
        <div className="space-y-4">
          {/* Briefing da marca gerado pela Izzi — colapsado por padrão */}
          {briefingsMarca.map((doc) => (
            <BriefingMarcaCard
              key={doc.id}
              secao={doc}
              clienteId={clienteId}
          ...
          {CATEGORIAS_ESTRATEGIA.map((cat) => {
            const secao = secoesEditaveis.find(
              (s) => s.categoria === cat.value && !s.subcategoria
            ) ?? secoesEditaveis.find((s) => s.categoria === cat.value) ?? null
            return (
              <SecaoEditor
                key={cat.value}
                clienteId={clienteId}
      ...
      {subTab === 'conteudo' && (
        <div className="space-y-4">
          {CATEGORIAS_CONTEUDO.map((cat) => {
            const secao = secoesEditaveis.find((s) => s.categoria === cat.value) ?? null
            return (
              <SecaoEditor
                key={cat.value}
                clienteId={clienteId}
...
// ---------------------------------------------------------------------------
// BriefingMarcaCard — briefing isolado por marca (gerado pela Izzi)
// ---------------------------------------------------------------------------

function BriefingMarcaCard({
  secao,
  clienteId,
  marcaId,
  podeEditar,
  ...
  function handleContinuar() {
    ...
    startContinuar(async () => {
      const res = await actionContinuarBriefingMarca(clienteId, marcaId)
      if (res.error) { setErro(res.error); return }
...
// ---------------------------------------------------------------------------
// SecaoEditor — edição inline de uma seção
// ---------------------------------------------------------------------------
    ...
    <div className="rounded-2xl border border-zinc-200 bg-white">
      <div className="flex items-center justify-between border-b border-zinc-100 px-5 py-3">
        <div className="flex items-center gap-2">
          ...
          {secao && <span className="text-[10px] text-zinc-400">v{versao}</span>}
          {geradoPorAgente && (
      ...
      <div className="p-5">
        {editando ? (
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-2">
              <input
                id={`visivel-${categoria}-${marcaId}`}
                type="checkbox"
                checked={visivelCliente}
                onChange={(e) => setVisivelCliente(e.target.checked)}
                className="rounded border-zinc-300"
              />
              <label htmlFor={`visivel-${categoria}-${marcaId}`} className="text-xs text-zinc-500">
...
// ---------------------------------------------------------------------------
// AprendizadosSection — insights de IA para este cliente
// ---------------------------------------------------------------------------
...
function AprendizadosSection({
    ...
    startTransition(async () => {
      ...
      if (res.error) {
        ...
        setInsights((prev) => {
          ...
          const entry = {
            id: novo.id, agentChave: novo.agentChave, agentNome: novo.agentNome,
            resumo: novo.resumo, taxaAprovacao: novo.taxaAprovacao,
            totalFeedbacks: novo.totalFeedbacks, padroesPositivos: novo.padroesPositivos,
            padroesNegativos: novo.padroesNegativos, sugestoes: novo.sugestoes,
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-zinc-500">Padrões extraídos via IA dos feedbacks de execuções para este cliente.</p>
      {erro && <p className="rounded-xl bg-red-50 px-4 py-3 text-sm text-red-600">{erro}</p>}
      <div className="space-y-4">
        {insights.map((insight) => {
          const taxa = insight.taxaAprovacao
          const taxaCor = taxa == null ? 'text-zinc-400' : taxa >= 70 ? 'text-green-600' : taxa >= 40 ? 'text-amber-600' : 'text-red-600'
          return (
            <div key={insight.id} className="rounded-2xl border border-zinc-200 bg-white p-5">
              <div className="mb-4 flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-0.5 text-[11px] text-zinc-400">{insight.totalFeedbacks} feedbacks avaliados</p>
                </div>
              ...
              {insight.resumo && (
                <p className="mb-4 border-l-2 border-zinc-200 pl-3 text-xs leading-relaxed italic text-zinc-500">{insight.resumo}</p>
              )}
              <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <ul className="space-y-1">
                      {insight.padroesPositivos.map((p, i) => (
                        <li key={i} className="flex items-start gap-1.5 text-xs text-zinc-600"><span className="mt-0.5 text-green-500">•</span>{p}</li>
                      ))}
                    </ul>
                  </div>
                )}
                  ...
                  <div>
                    ...
                    <ul className="space-y-1">
                      {insight.padroesNegativos.map((p, i) => (
                        <li key={i} className="flex items-start gap-1.5 text-xs text-zinc-600"><span className="mt-0.5 text-red-400">•</span>{p}</li>
                      ))}
                    </ul>
                  </div>
                )}
                  ...
                  <div>
                    ...
                    <ul className="space-y-1">
                      {insight.sugestoes.map((s, i) => (
                        <li key={i} className="flex items-start gap-1.5 text-xs text-zinc-600"><span className="mt-0.5 text-violet-400">•</span>{s}</li>
                      ))}
                    </ul>
                  </div>
                )}
              ...
              {podeEditar && (
                <div className="mt-4 flex items-center justify-between border-t border-zinc-100 pt-3">
                  <p className="text-[10px] text-zinc-400">
                    Atualizado em {new Date(insight.atualizadoEm).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short', year: 'numeric' })}
                  </p>
                  <button
