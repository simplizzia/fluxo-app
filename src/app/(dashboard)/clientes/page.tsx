...
import { buscarClientes, type ClienteListItem } from './actions'
...
export default async function ClientesPage() {
  ...
  const arquivados = lista.filter((c) => c.status === 'inativo')
      ...
      {lista.length === 0 ? (
        ...
        <div className="overflow-hidden rounded-2xl border border-zinc-200 bg-white">
          <div className="divide-y divide-zinc-100">
            {ativos.map((cliente) => <ClienteRow key={cliente.id} cliente={cliente} />)}
          </div>
        </div>
      )}
      ...
      {arquivados.length > 0 && (
        <div>
          ...
          <div className="overflow-hidden rounded-2xl border border-zinc-200 bg-zinc-50/60">
            <div className="divide-y divide-zinc-100">
              {arquivados.map((cliente) => <ClienteRow key={cliente.id} cliente={cliente} arquivado />)}
            </div>
          </div>
        </div>
      )}
...
function ClienteRow({ cliente, arquivado }: { cliente: ClienteListItem; arquivado?: boolean }) {
  const statusCfg = STATUS_LABEL[cliente.status] ?? STATUS_LABEL.inativo
  return (
    <Link
      href={`/clientes/${cliente.id}`}
      className={`flex items-center gap-4 px-5 py-4 transition hover:bg-zinc-50/70 ${arquivado ? 'opacity-70' : ''}`}
    >
      <div className={`flex h-9 w-9 flex-none items-center justify-center rounded-xl text-sm font-bold text-white ${arquivado ? 'bg-zinc-400' : 'bg-gradient-brand'}`}>
